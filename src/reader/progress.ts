...
export function updateProgress(currentPage: number, totalPages: number): void {
  ...
  if (totalPages <= 1) {
    // 单页或无效：设置为 100%
    $("#reader-progress-fill").css("width", "100%");
    $("#reader-progress-thumb").css("left", "calc(100% - 1mm)");
    return;
  }
...
function handleProgressBarClick(event: JQuery.Event): void {
  ...
  // 计算目标页码，确保不超出范围
  var targetPage = Math.floor(ratio * (state.totalPages - 1));
  targetPage = Math.max(0, Math.min(targetPage, state.totalPages - 1));
