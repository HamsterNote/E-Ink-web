...
export function renderPage(pageNumber: number): void {
	...
	// 提取该页的内容
	var pageContent = state.content.substring(pageInfo.startIndex, pageInfo.endIndex);
...
export function splitContentIntoPages(content: string): PageInfo[] {
	...
	while (currentIndex < contentLength) {
		...
		// 检查是否进入新章节（通过查找 <h1> 标签）
		var remainingContent = content.substring(currentIndex);
		var h1Match = remainingContent.match(/<h1/i);
 ...
 * @returns 结束位置
 */
function findPageEnd($measurer: JQuery<HTMLElement>, startIndex: number, maxHeight: number): number {
 ...
 * @returns 断点位置
 */
function findBestBreakPoint(content: string, position: number, maxHeight: number, $measurer: JQuery<HTMLElement>): number {
	// 定义优先级：段落 > 句子 > 单词
	var breakPatterns = [
