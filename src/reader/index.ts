...
import $ from 'jquery';
import { initState, getState, clearState, subscribe, saveProgress } from './state';
import { getBookContent } from './api';
import { renderPage, calculateTotalPages, splitContentIntoPages, clearPageCache } from './renderer';
...
export function showReader(bookUuid: string): void {
	...
	// 获取书籍内容
	getBookContent(bookUuid).then(function(response) {
		// 隐藏加载提示
		hideLoading();
		...
		if (state) {
			...
			// 更新书签按钮状态
			var hasBookmarkInCurrentPage = state.bookmarks.indexOf(state.currentPage) !== -1;
