 ...
 * @param callback 回调函数
 */
export function setShowLoginModalCallback(callback: (showClose: boolean) => void): void {
 ...
 * @param onError 错误回调
 */
export function getUserInfo(callback: (userInfo: UserInfo) => void, onError?: () => void): void {
	$.ajax({
		url: '/api/v1/users/me',
		type: 'GET',
		data: {},
...
export function uploadFile(
	...
	$.ajax({
		...
		contentType: 'application/json',
		dataType: 'json',
		success: function(initRes: { uuid?: string; id?: string; data?: string; result?: { uuid?: string } }) {
			try {
				...
				xhr.onload = function() {
					...
					if (status >= 200 && status < 300) {
						...
						if (onError) {
							onError({ message: 'upload failed', status: status, body: xhr.responseText });
						}
					}
				};
				// 发送文件本体
...
export function getFileList(
	...
	return new Promise(function(resolve, reject) {
		...
		$.ajax({
			...
			dataType: 'json',
			success: function(response) {
				var items = response.items.map(function(item: Omit<BookFile, 'pageSize' | 'page'>) {
					return {
						size: item.size,
 ...
 * 获取目录列表（占位函数）
 */
export function getDirectoryList(): void {
	// TODO: 实现目录列表获取
}
